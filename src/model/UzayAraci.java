 ...
 * @since   20.04.2025
 *  * <p>
 *  UzayAraci sınıfı; aracın adını, çıkış ve varış gezegenlerini,
 *  çıkış tarihini, toplam mesafesini, kalan mesafesini, aktif/tamamlandı/İMHA
 *  durumlarını ve varış tarihini tutar. Aktivasyon ve saat ilerletme
 *  metodları içerir, ayrıca raporlama için durum ve satır biçimlendirme sağlar.
 * </p>
 */
package model;
...
public class UzayAraci {
	...
	public void birSaatIlerle() {
		if (aktif && !tamamlandi) {
			this.kalanMesafe = Math.max(0, this.kalanMesafe - 1);
			if (kalanMesafe <= 0) {
				aktif = false;
				tamamlandi = true;
			}
	...
	public String[] raporSatiri() {
		...
		String kalan = isImha() ? "--"
				: hedefeUlasti() ? "0" : isAktif() ? String.valueOf(getKalanMesafe()) : String.valueOf(getMesafe());
	...
	public String getDurumYazili() {
		if (isImha())
			return "İMHA";
		if (hedefeUlasti())
			return "ULAŞTI → " + getVarisTarihi();
		if (isAktif())
			return "YOLDA → Kalan: " + getKalanMesafe() + " saat";
		return "BEKLEMEDE";
	}
