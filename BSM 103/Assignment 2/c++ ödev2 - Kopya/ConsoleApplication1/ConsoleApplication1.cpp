...
short ReadNumber(string message, short from, short to, string Error_message) {
    ...
    do {
        cout << message << endl;
        cin >> number;
        if (number<from || number>to || !cin.good()) {
            cin.ignore(numeric_limits<streamsize>::max(), '\n');
            cout << Error_message << endl;
        }
    } while (number<from || number>to || !cin.good());
    return number;
}
struct user
{
...
void SecilenSayiYonlendirme()
{
    ...
    while (1)
    {
        ...
        short numara  = ReadNumber("Please enter the transaction number you want to make :", 1, 10, "only 1-10");
